...
//#define USE_CAMERA_SERVICE_INTERFACE
...
private:
    static int                  mFd; //touch input device file point
    static VirtualTouch*        mVirtualTouch;

#ifndef USE_CAMERA_SERVICE_INTERFACE
private:
                          ...
                          void *user)
    {
        ...
        reportMockPoint(i, mFd);
#else
        writeTouchPoints(mVirtualTouch, pBase, mFd);
#endif
...
int CmdImp::mFd = NULL;
VirtualTouch* CmdImp::mVirtualTouch = NULL;
...
onParseArgumentCommand(Vector<String8>& rvCmd, sp<Argument> pArgument)
{
    ...
    pArgument->mOpenId = 0;
    pArgument->mPreviewSize = android::Size(640, 480);
...
threadLoop()
{
    ...
    mFd = ::open("/sys/bus/platform/drivers/asu-fan/asu-fan/pointset", O_WRONLY | O_NDELAY);
    if(mFd < 0) {
        MY_LOGE("open touch input device file error, mFd=%d", mFd);
        goto lbExit;
    }
    ...
    if(!mVirtualTouch->init(CALIBRATION_PATH, 1280, 720, mpArgument->mPreviewSize.width, mpArgument->mPreviewSize.height)) {
        MY_LOGE("init virtual touch error");
        goto lbExit;
    }

    // 3. Start camera preview
#ifdef USE_CAMERA_SERVICE_INTERFACE
